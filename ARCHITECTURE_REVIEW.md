...
> 评估对象：`C:\Users\HUAI\Desktop\UE5文档\`（现已改名 DocAtlas）
> 评估时间：2026-07-25
> 依据：实际代码 5,717 行、实际数据库 739 MB、实际目录与配置文件。
> **本报告写作时不含任何代码改动。** 唯一已执行的写操作是建立 Git 仓库（见第 0 节）。

---

## 执行状态（2026-07-26 追记）

本报告的方案已获批准并执行完毕。这一节只记录**实测结果**，
下文其余部分保持评估当时的原样，不回填修改——那是当时看到的事实，
留着才能对照出哪些判断准、哪些判断偏了。

| 报告章节 | 落地情况 | 实测验证 |
|---|---|---|
| 第 14 节 阶段 0 | 冷备份 + `parser_version` | `integrity_check: ok`，行数全对 |
| 第 14 节 阶段 1 | 代码搬出数据目录 | 主入口 11 个动作实测正常 |
| 第 14 节 阶段 2 | 适配器 / 领域知识分层 | **329 页加工指纹逐字相同**（零行为改变） |
| 第 14 节 阶段 3 | 分块合并 + 全量重切 | 见下 |
| 第 12 节 | MCP Server | 协议握手 / 工具调用 / 错误处理实测通过 |
| 第 5 节风险 | Skill 写死路径 | 改为安装时生成，旧技能已移出 |

### 判断准的地方

- **"一页都不用重抓"**——成立。全部重切都是本地重放，全程没有一个网络请求。
- **"加一个版本 = 复制一份 toml"**——成立。实测复制配置改 4 行即得到
  独立的 UE 5.9 数据集，代码零改动。
- **问题 1（无法自证出身）确实是最重要的那个**。加上版本列之后，
  `reprocess` 顺带变成可续传的；这次重切中途我改了一次代码，
  正因为有版本列，才能精确定位到受影响的只有 2 页，而不是全库重做。

### 判断偏了的地方

**一、把"碎块数量"当成了问题本身。**

报告预测碎块比例会从 54.6% 降到 15% 以下。第一版改完只降到 26.9%，
一度以为没做到。实际拆开看，剩下的小块大部分是**整页本来就只有这么点
内容**的蓝图节点页——它们是完整的一条知识，不是碎片。真正该盯的指标是
"一页被切成好几个都答不了问题的小块"，这个数字是 **7,860 → 76**。

**二、按小节的标题名判断它有没有用——这个错得比较重。**

第一版看到 6,769 个碎块是 `navigation` 类型，就整类排除出知识库，
理由写得很顺："面包屑而已，信息 `heading_path` 里已经有了。"

**没有打开看过内容。** Epic 蓝图页的 `Navigation` 小节实际长这样：

```
## Navigation
[BlueprintAPI](…) > [BlueprintAPI/Abilities](…)     ← 面包屑
Returns true if any passed in tags are blocked      ← 这个节点是干嘛的
Target is Ability System Component                  ← 作用在哪个类型上
```

后两行是整页最有用的信息。丢掉的直接后果：每个蓝图节点的功能描述从
知识库里消失；全站 1,407 条 `Target is X` 声明，chunks 里只剩 59 条。

这个错是靠**验收脚本查交叉关系分布**发现的——所有健康检查都通过了，
唯独关系统计里数字对不上。如果只看"跑完没报错"，这个损失会一直躺在库里。

改法不是留着它单独成块（那还是几千条半噪音），而是让它并进同页其他内容，
同时不由它决定合并块的类型。结果比丢掉它好得多：

| | 改造前 | 丢弃导航（错） | 修正后 |
|---|---:|---:|---:|
| <50 token 碎块 | 54.6% | 26.4% | **5.2%** |
| 平均块大小 | 143 | 265 | **273** |
| 含 `Target is` 的块 | 1,407 | 59 | **1,408** |

**三、报告没预料到的：孤块尾巴。**

大段落切分后剩的一小截（有的只有 20 token）会独立成块。这是原有切分
逻辑的缺陷，跟合并策略无关，实际动手才看到，顺手一起修了。

**四、合并把一条正则的隐含前提抽掉了——第二个真错。**

`Target is X` 里的 X 到哪个词结束？原来的正则用"句号 / 两个空格 / **字符串结尾**"
收边。它一直能用，靠的是一个从没写下来的前提：**这句正好是那个小节的最后一句**。

合并之后前提没了。`content_text` 把空白压成单空格，实际长这样：

```
...are blocked Target is Ability System Component Inputs Type Name Description...
                         └─ 名字 ─┘└─ 紧接着就是下一段，中间没有任何标点
```

命中数从 1,336 掉到 4，`document_statement` 关系 40 → 0。

改法是**不再猜边界**：把后面的词从长到短拿去别名表里对，第一个对得上已知实体的
就是它（从长到短是必须的，`Actor Component` 得赢过 `Actor`）。修完 40 条一条不差
地回来了。这条比第二个错更隐蔽——它不在我改的文件里，是被我改的东西**间接**打断的。

抓到它的是上一条教训留下的那个检查（"每类本该产出的证据都得真有货"）。
上一次它救的是自己那个错，这一次救的是一个当时还不存在的错。
...
|---|---:|---:|
| 被切碎的页面 | 7,860 | 76 |
| 平均块大小 | 143 token | 273 token |
| <50 token 碎块 | 54.6% | 5.2% |
| 知识块总数 | 48,099 | 25,552 |
| 含 `Target is` 的块 | 1,407 | 1,408 |
| `document_statement` 关系 | 40 | 40 |
| 测试用例 | 44 | 75 |
| 切分规则版本 | （没有这个概念） | v3 |
