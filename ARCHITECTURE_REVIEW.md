...
报告预测分块合并后碎块比例会从 54.6% 降到 15% 以下，**实际只降到 26.9%**。

原因是当时把"碎块数量"当成了问题本身。实际拆开看，剩下的小块里大部分
是**整页本来就只有这么点内容**的蓝图节点页（一个输入、一个输出），
它们是完整的一条知识，不是碎片。真正该看的指标是"一页被切成好几个
都答不了问题的小块"——这个数字是 **7,860 → 77**，降了 99%。

另外报告没预料到两件事，是实际动手才发现的：

- **导航小节**（面包屑）占了 6,769 个碎块。它的信息 `heading_path` 里
  已经完整表达，检索里也一直在给它扣分——不该进知识库。
- **孤块尾巴**：大段落切分后剩的一小截（有的只有 20 token）会独立成块。
  这是原有切分逻辑的缺陷，跟合并策略无关，顺手一起修了。
