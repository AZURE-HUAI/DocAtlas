...
本知识库用于回答 Unreal Engine 5.8 的问题。所有事实优先从本地库取回，
并把结果里的 `source_url` 作为 Epic DOC 原出处引用给用户。

装好的 Claude Code 技能位于 `~/.claude/skills/ue5-docs/`，
下面是它遵循的完整规则。
...
```powershell
python ue58_docs.py ask "Set Timer by Function Name" --token-budget 3000
```

`ask` 一步到位地返回**已裁剪、已去重、已排序**的 Markdown：命中的知识块正文、
每块的 Epic 原出处、以及一跳交叉关系的指针。绝大多数问题到这里就结束了。

**本地没有的页面 `ask` 会自动补抓**（全站清单早已冻结，所以知道去哪取），
通常一两秒。所以不要因为"可能还没抓到"就绕开它，也不要自己去联网找 Epic 文档。
...
- ❌ 用 Read 打开 `ue58_docs.sqlite3` / `manifest.jsonl` / `site_inventory.jsonl`
- ❌ 用 grep 扫整个知识库目录
...
| 证据 | 置信度 | 转述时怎么说 |
|---|---|---|
| `official_link` | 1.0 | "Epic 文档中直接链接到" |
...
- 明确限定 UE 5.8，不要把其他版本的行为混进来。
- 每个关键结论旁边附对应的 Epic DOC 原出处 URL，不要自己拼 URL。
