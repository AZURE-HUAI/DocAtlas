...
4. 在档案中记录 GitHub Issue 链接，在 GitHub Issue 中回链档案。
...
4. 需要在线协作时再创建 GitHub Issue，并建立双向链接。
...
该检查验证编号唯一性、必填元数据、目录与生命周期的一致性、关联编号、索引覆盖和
相对链接。GitHub Actions 只在相关协作文件变化时运行同一检查。
