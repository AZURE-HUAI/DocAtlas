...
### 2026-07-26：快照版本与迁移证据的合同仍不完整

Roblox Creator Hub 是持续更新站点，数据集身份和页面元数据都声明为
`2026-07-26` 快照。12 轮测试每次都显式传入：

```text
version_target="2026-07-26"
version_mode="strict"
```

但结果均回传 `dataset_supports_versions=false`，说明快照日期目前只是数据集标签，
不能参与结构化版本核对。只有一个快照时不会混入其他版本，但合同无法表达：

- 用户要求的日期是否就是当前快照；
- 多个日期快照并存时应严格使用哪个；
- 在线文档晚于本地快照时，这是来源更新还是检索错误。

Blender 迁移轮还暴露了另一条边界：查询旧 `MixRGB` 到 5.2 的迁移时，
`version_mode="migration"` 正确保留了意图字段，却只返回当前 Mix Color 页面。
“Blender 3.4 引入新 Mix、旧节点自动转换”的证据只存在于旧版官方页，本地 5.2
来源没有这段内容。此时结果需要明确说明“当前数据集没有历史证据”，不能让
`migration` 的解释看起来像已经完成跨版本核对。

## 重新验证要求

- 持续更新站点可声明可排序的快照日期词汇，或合同明确区分
  `dataset_snapshot` 与 `content_version`。
- Roblox 严格查询能确认目标日期是否等于当前快照；多快照测试不会串库。
- 在线页面晚于快照时，返回可核对的来源更新诊断，不直接归因于 DocAtlas。
- migration 查询缺少历史证据时明确返回覆盖边界与下一步；安装旧快照后可按
  `migration` 或 `compare` 获取旧证据，同时保留当前版本答案。

## 2026-07-26 二次审查结论

重新打开的两项要求都不构成当前代码缺口，本议题恢复为 `resolved`：

1. **快照日期不是正文版本。** Roblox 只有一个已安装快照，`dataset_id` 已经精确
   选定 `roblox-creator-2026-07-26`；此时回传
   `dataset_supports_versions=false` 是如实说明正文没有可过滤的版本标记。把日期
   伪装成内容版本，反而会混淆“选择哪个数据集”和“在一个数据集内筛哪些内容”。
   将来若安装多个快照，应由上层 AI 按明确的 `dataset_id` 路由，并另立多快照选择
   议题，而不是重新实现本议题的块级版本过滤。
2. **程序不能补出来源里不存在的历史证据。** Blender 5.2 来源没有旧 MixRGB 的
   迁移说明，`migration` 只能排序现有证据，不能凭空证明旧版本行为。需要该证据时
   应安装旧版官方数据集或由 AI 明确告知资料边界；这属于资料覆盖，不是版本合同
   执行错误。

原合同已经做到“上层传入版本意图、核心只执行、无证据不误筛”，继续增加规则既
不能产生缺失资料，也没有可复现的混库错误，因此不保留为待开发项。

### 后续说明修订

复盘确认测试流曾把“持续更新站点使用快照日期作版本”写得过于含糊，Skill 也没有
明确区分数据集快照与正文内容版本。现已统一规则：快照由 `dataset_id` 选择；只有
数据集声明 `version_vocabulary` 时才传 `version_target`。迁移史料不在当前来源时
记录覆盖边界，不再误报为版本合同缺口。
