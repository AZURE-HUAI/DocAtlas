...
`docatlas_search` 的 `match_stage` 说明这一条是**怎么**中的：`entity` 表示库里有
一页正好叫这个名字，其余档都是关键词碰上的邻居。同名概念最容易串台（`For Loop`
既是节点名也是通用说法），`entity` 那条才是官方同名页，不要和后面几条并列转述。
`docatlas_ask` 不返回这个字段，要分辨就先 `search`。
