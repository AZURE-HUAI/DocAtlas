...
`--sample-per-category N` 是**每一类最多 N 页**，不是总共 N × 类数：某一类
只有 9 页时，就抓 9 页，缺的额度不会转到别的类去。已经抓成功的算在这一类的
额度里，所以同一条命令跑第二遍不会越抓越多。

**2. 每一步都验收，而且要看数字。** "命令跑完没报错"在这个项目里骗过三次了。
`validate` 输出的是 JSON，逐项看 `status`，别只看退出码。

尤其看这两项，它们专门抓"空得很整齐"的库：

- `inventory_not_empty` —— 一个页面都没有的空库不算通过。
- `declared_categories_have_pages` —— 配置声明了某一类，却一页都没枚举到，
  几乎总是 `[categories]` 的匹配片段写错了。确实可能为空的分类写进
  `optional_categories`，别为了让它变绿就把检查删掉。
...
- **怎么知道有哪些页面**：有没有 `sitemap.xml`？没有的话，有没有分页 API、
  目录页，或者像 Sphinx `searchindex.js` 那样的静态索引？
...
**站点没有 sitemap 时**，再多实现这两个，前两行就不用写了：

| 函数 | 干什么 |
|---|---|
| `inventory_feeds(dataset)` | 返回 `[(清单入口地址, 分类或 None)]` |
| `read_feed(dataset, url)` | 把一个入口读成 `[(分类或 None, 页面地址)]` |

翻页（continuation）、限流和重试都在 `read_feed` 里自己处理——那是"这个站
怎么列页"的一部分。并发、写库、失败诊断、`inventory` 验收一律照旧复用，
不需要动核心。分类优先取条目自己给的，其次才是入口所属的，所以一个入口
列出多个分类也能表达。
...
| 现象 | 多半是 | 怎么办 |
|---|---|---|
| 清单枚举出 0 页 | 清单入口地址或分类片段不对 | `validate` 的 `inventory_not_empty` 会直接报出来；手动打开入口看结构，对 `[categories]` |
